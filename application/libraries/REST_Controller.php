...
class REST_Controller extends CI_Controller
{
	...
	public function _remap($method, $params = array())
	{
		try
		{
			...
			$formatted_data = $this->format($this->response_data);			
	...
	protected function _detect_method()
	{
		...
		elseif ($method == 'POST' && $this->input->server('CONTENT_TYPE') == 'application/x-www-form-urlencoded')
		{
			$this->params = $_POST;
		}
		else
        {
            ...
            if(strlen($str) > 0){
                $obj = json_decode($str) ; //json 형태로 넘어오는 경우
            }

            if(json_last_error() == JSON_ERROR_NONE ){ //json이면
                $this->params = get_object_vars($obj) ; 
            } else {
    ...
    protected function _format_json($object) { 
        return json_encode($object); 
    }

    protected function _format_php($object) { 
    ...
    public function set_response_data($data){
        $this->response_data = array('result'=>$data) ; 
    }
}
